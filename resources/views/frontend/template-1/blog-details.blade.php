                ...
                <div class="col-lg-8">
                    <div class="post-thumb mb-30">
                        @if (fileExists('uploads/blog/', $blog_details->image) != false && $blog_details->image != null)
                            <img class="img-fluid" src="{{ asset('uploads/blog/' . $blog_details->image) }}"
                                alt="{{ $blog_details->title }}">
                        @else
                            <img src="{{ asset('uploads/author-cover-placeholder.webp') }}" alt="{{ $blog_details->title }}">
                        @endif
                    </div>
                    <div class="post-title mb-40">
                        <h1>{{ $blog_details->getTranslation('title') }}</h1>
                    </div>
                    <div class="blog-meta two mb-50">
                        <div class="author-area">
                            <div class="author-img">
                                @if ($blog_details->users?->image)
                                    <img src="{{ asset('uploads/users/' . $blog_details->users?->image) }}"
                                        alt="{{ $blog_details->users?->username }}">
                                @else
                            ...
                            <div class="author-content">
                                <h6>{{ translate('By') }}, <a href="#">
                                        {{ $blog_details->users?->fname ? $blog_details->users?->fname . ' ' . $blog_details->users?->lname : '' }}</a>
                                </h6>
                            </div>
                        </div>
                        <ul>
                            <li>
                                ...
                                {{ calculate_read_time(strip_tags($blog_details->description)) }}
                                {{ translate('Min Read') }}
                            </li>
                            <li>
                                <a href="#comment-area">
                                    ...
                                    {{ $blog_details->comments?->count() < 9 && $blog_details->comments?->count() != 0 ? '0' . $blog_details->comments?->count() : $blog_details->comments?->count() }}
                                    {{ $blog_details->comments?->count() > 1 ? 'Comments' : 'Comment' }}
                                </a>
                            </li>
                        </ul>
                    </div>
                    ...
                    <div class="tag-and-social-area mb-30">
                        ...
                        <div class="social-area">
                            ...
                            <ul class="social-link">
                                ...
                                <li>
                                    <a
                                        href="https://twitter.com/intent/tweet?url={{ url('blog/' . $blog_details->slug) }}&text={{ $blog_details->getTranslation('name') }}"><svg
                                ...
                                <li>
                                    <a
                                        href="https://www.linkedin.com/shareArticle?url={{ url('blog/' . $blog_details->slug) }}&title={{ $blog_details->getTranslation('name') }}"><i
                    ...
                    @include('frontend.template-1.partials.blog-comment')
                </div>
                <div class="col-lg-4">
                            ...
                            <div class="single-widget mb-30">
                                ...
                                <ul class="category-list">
                                    @foreach ($categories as $category)
                                        <li>
                                            <a href="{{ url('blog/category/' . $category->slug) }}">{{ $category->getTranslation('name') }}
                                                <span>({{ $category->blogs->count() ?? 0 }})</span>
                                            </a>
                                        </li>
                                    @endforeach
                                </ul>
                            ...
                            @foreach ($recentBlogs as $recentBlog)
                                <div class="recent-post-widget mb-20">
                                    ...
                                    <div class="recent-post-content">
                                        <a
                                            href="{{ url('blog/' . $recentBlog->slug) }}">{{ date('F d, Y', strtotime($recentBlog->created_at)) }}</a>
                                        <h6><a
