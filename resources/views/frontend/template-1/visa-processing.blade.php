@php
    ...
    if (isset($singelWidgetData->widget_content)) {
        ...
        $limit = $widgetContent['display'] ? $widgetContent['display'] : 6;
        $orderBy = $widgetContent['order_by'] ? $widgetContent['order_by'] : 'desc';
    }

    $visas = visa($limit, '', $orderBy);
   

@endphp
@if($visas)
<div class="visa-section mb-120 mt-60">
                    <img src="{{asset('frontend/img/home1/section-vector2.png')}}" alt="" class="section-vector2">
    <div class="container">
        <div class="row">
                    ...
                    <div class="swiper-wrapper">
                        ...
                        <div class="swiper-slide">
                            <div class="package-card2">
                                <a href="{{route('visa.details',$item->slug)}}"><img src="{{asset('uploads/visa/features/'.$item->features_image)}}" alt="{{$item->getTranslation('title')}}"></a>
                                <div class="eg-tag">
                                    <h4><a href="{{route('visa.details',$item->slug)}}">{{$item->countries?->name}}</a></h4>
                                </div>
                                <div class="package-card2-content">
                                    <div class="title">
                                        <h6>{{$item->getTranslation('title')}}</h6>
                                    </div>
                                    <div class="price-area">
                                        <span>{{currency_symbol().$item->cost}}</span>
