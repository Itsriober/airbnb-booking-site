...
@if(isset($aboutContentData))
<!-- Start Home1 About section -->
    <div class="home1-about-section mb-100 mt-100">
        <img src="{{asset('frontend/img/home1/section-vector1.png')}}" alt="bg" class="section-vector1">
        <div class="container">
            <div class="row">
                <div class="col-lg-6">
                    <div class="about-content">
                        <div class="section-title mb-40">
                            <span>
                                ...
                                {{isset($aboutContentData['about_main_title']) ? $aboutContentData['about_main_title'] : translate('About Us')}}
                                <svg xmlns="http://www.w3.org/2000/svg" width="15" height="16" viewBox="0 0 15 16">
                                    <path fill-rule="evenodd" clip-rule="evenodd"
                            ...
                            <h2>{{isset($aboutContentData['about_title']) ? $aboutContentData['about_title'] : ''}}</h2>
                        </div>
                        @if(isset($aboutContentData['tabs']))
                        <ul class="nav nav-pills" id="pills-tab3" role="tablist">
                            @foreach($aboutContentData['tabs'] as $key=>$tabs)
                            <li class="nav-item" role="presentation">
                                <button class="nav-link @if($key === array_key_first($aboutContentData['tabs'])) active @endif" id="mission-tab" data-bs-toggle="pill"
                                    data-bs-target="#tabs{{$key}}" type="button" role="tab" aria-controls="mission" aria-selected="true">
                                    <i class="{{isset($tabs['tabs_icon']) ? $tabs['tabs_icon'] : ''}}"></i>
                                    {!! isset($tabs['tabs_name']) ? $tabs['tabs_name'] : '' !!}
                                </button>
                            </li>
                            @endforeach
                        </ul>
                        <div class="tab-content" id="pills-tab3Content">
                            ...
                            @endphp
                            <div class="tab-pane fade @if($key === array_key_first($aboutContentData['tabs'])) show active @endif" id="tabs{{$key}}" role="tabpanel">
                                {!! clean($description) !!}
                            </div>
                            @endforeach
                        </div>
                        ...
                        <div class="about-content-bottom">
                            @if(isset($aboutContentData['btn_url']))
                            <a href="{{isset($aboutContentData['btn_url']) ? $aboutContentData['btn_url'] : ''}}" class="primary-btn1">{{isset($aboutContentData['btn_text']) ? $aboutContentData['btn_text'] : translate('More About')}}</a>
                            @endif
                            <div class="counter-area">
                                @if(isset($aboutContentData['customer_img']))
                                <div class="customer-img-grp">
                                    <img src="{{('uploads/about_content/'.$aboutContentData['customer_img']['img'])}}" alt="customer image">
                                </div>
                                @endif
                                @if(isset($aboutContentData['customer_number']))
                                <div class="content">
                                    <div class="number">
                                        <h6 class="counter">{{$aboutContentData['customer_number']}}</h6>
                                        <span>+</span>
                                    </div>
                ...
                <div class="col-lg-6 d-flex justify-content-end">
                    @if(isset($aboutContentData['about_img']['img']))
                    <div class="about-img">
                        <img src="{{('uploads/about_content/'.$aboutContentData['about_img']['img'])}}" alt="">
                        <img src="{{asset('frontend/img/home1//vector/about-img-bg-vector.svg')}}" alt="" class="vector">
                    </div>
                    @endif
                </div>
            </div>
