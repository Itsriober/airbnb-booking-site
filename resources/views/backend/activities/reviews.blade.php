...
@section('content')
        ...
        <div class="col-12">
            <div class="table-wrapper">
                <table class="eg-table table customer-table">
                    ...
                    <tbody>
                        @if ($activitiesReviews->count() > 0)
                            @foreach ($activitiesReviews as $key => $productReview)
                                <tr>
                                    <td data-label="S.N">
                                        {{ ($activitiesReviews->currentpage() - 1) * $activitiesReviews->perpage() + $key + 1 }}
                                    </td>
                                    <td data-label="Customer" class="text-start"><a
                                            href="{{ route('customer.view', $productReview->users->id) }}">{{ $productReview->users->fname . ' ' . $productReview->users->lname }}<br><span
                                                class="text-purple">{{ '@' . $productReview->users->username }}</span></a>
                                    </td>
                                    <td data-label="Email / Phone"> <a
                                            href="mailto:{{ $productReview->users->email }}">{{ $productReview->users->email }}</a>
                                        <br><a href="tel:{{ $productReview->users->phone }}"
                                            class="phone">{{ $productReview->users->phone }}</a></td>
                                    <td data-label="Rate">
                                    ...
                                    <td data-label="Review">{{ $productReview->review }}</td>
                              
                                    <td data-label="Status">
                                        <div class="form-check form-switch">
                                            <input class="form-check-input flexSwitchCheckStatus" type="checkbox"
                                                data-activations-status="{{ $productReview->status }}"
                                                data-id="{{ $productReview->id }}" data-type="activitiesReview"
                                                id="flexSwitchCheckStatus{{ $productReview->id }}"
                                                {{ $productReview->status == 1 ? 'checked' : '' }}>
                                        </div>
                                    </td>
                                    ...
                                    <td data-label="Date">
                                        ...
                                        <span
                                            class="time">{{ date('h.i A', strtotime($productReview->created_at)) }}</span>
                                    </td>
                                    <td data-label="Reply">
                                        ...
                                        @else
                                            <button type="button" data-review_id="{{ $productReview->id }}"
                                                data-product_id="{{ $productReview->product_id }}"
    ...
    @include('backend.activities.modal')
    @push('footer')
        <div class="d-flex justify-content-center custom-pagination">
            {!! $activitiesReviews->links() !!}
        </div>
    @endpush
@endsection
