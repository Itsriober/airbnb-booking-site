...
.select2-container {
    ...
    <div class="row mb-35">
        <div class="page-title d-flex justify-content-between align-items-center">
            ...
            <button type="button" class="eg-btn btn--primary back-btn me-1" data-bs-toggle="modal"
                ...
                <a href="{{route('hotel.attribute.list')}}" class="eg-btn btn--primary back-btn"> <img src="{{asset('backend/images/icons/back.svg')}}" alt="{{ translate('Go Back') }}"> {{ translate('Go Back') }}</a>
            </div>
        </div>
    </div>
    @php
                            ...
                            <tr>
                                ...
                                <td data-label="Language">
                                    @foreach (\App\Models\Language::all() as $key => $language)
                                        ...
                                        @else
                                            <a
                                                href="{{ route('hotel.attribute.terms.edit', ['attribute_id' => $attribute_id,'id' => $term->id, 'lang' => $language->code]) }}"><i
                                ...
                                <td data-label="Option">
                                    <div
                                        ...
                                        <a class="eg-btn add--btn"
                                            href="{{ route('hotel.attribute.terms.edit', ['attribute_id' => $attribute_id,'id' => $term->id, 'lang' => get_setting('DEFAULT_LANGUAGE', 'en')]) }}"><i
                                                class="bi bi-pencil-square"></i></a>
                                        <form method="POST" action="{{ route('hotel.attribute.terms.delete', [$attribute_id,$term->id]) }}">
                                            @csrf
    ...
    @include('backend.hotels.attribute_terms.modal')
@endsection
