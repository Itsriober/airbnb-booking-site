                    ...
                    <div class="page-title d-flex justify-content-between align-items-center">
                        ...
                        <div class="language-changer">
                            ...
                            @foreach (\App\Models\Language::all() as $key => $language)
                                ...
                                @else
                                <a href="{{route('hotel.attribute.terms.edit',['attribute_id'=> $attribute_id, 'id'=>$termSingle->id, 'lang'=>$language->code] )}}"><img src="{{ asset('assets/img/flags/'.$language->code.'.png') }}" class="mr-3" height="16"></a>
                                @endif
                            @endforeach
                        </div>
                        <a href="{{route('hotel.attribute.terms.list',$attribute_id)}}" class="eg-btn btn--primary back-btn"> <img src="{{asset('backend/images/icons/back.svg')}}" alt="{{ translate('Go Back') }}"> {{ translate('Go Back') }}</a>
                    </div>
                </div>
                <div class="row">
                    ...
                    <form action="{{route('hotel.attribute.terms.update', ['attribute_id'=> $attribute_id, 'id'=> $termSingle->id])}}" method="POST" enctype="multipart/form-data">
                        ...
                        @csrf
                                ...
                                <div class="form-inner mb-25">
                                    ...
                                    @if($termSingle->image)
                                    <img src="{{asset('uploads/hotel/attribute/'.$termSingle->image)}}" alt="{{$termSingle->name}}" width="50">
                                    @endif
