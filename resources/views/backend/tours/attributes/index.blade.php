...
.select2-container {
    ...
    <div class="row mb-35">
        <div class="page-title d-flex justify-content-between align-items-center">
            ...
            <button type="button" class="eg-btn btn--primary back-btn me-1" data-bs-toggle="modal"
                ...
                <a href="{{route('tours.list')}}" class="eg-btn btn--primary back-btn"> <img src="{{asset('backend/images/icons/back.svg')}}" alt="{{ translate('Go Back') }}"> {{ translate('Go Back') }}</a>
            </div>
        </div>
    </div>
    @php
                    ...
                    <tbody>
                        ...
                        @foreach ($attributes as $key => $attribute)
                            <tr>
                                ...
                                <td data-label="Terms"><a href="{{ route('tour.attribute.terms.list', $attribute->id) }}" class="eg-btn primary-light--btn">{{ $attribute->tour_terms->count() }}</a></td>
                                <td data-label="Language">
                                    @foreach (\App\Models\Language::all() as $key => $language)
                                        ...
                                        @else
                                            <a
                                                href="{{ route('tour.attribute.edit', ['id' => $attribute->id, 'lang' => $language->code]) }}"><i
                                ...
                                <td data-label="Option">
                                    <div
                                        ...
                                        <a class="eg-btn add--btn"
                                            href="{{ route('tour.attribute.edit', ['id' => $attribute->id, 'lang' => get_setting('DEFAULT_LANGUAGE', 'en')]) }}"><i
                                                class="bi bi-pencil-square"></i></a>
                                        <form method="POST" action="{{ route('tour.attribute.delete', $attribute->id) }}">
                                            @csrf
                                        ...
                                        <a class="eg-btn account--btn"
                                            href="{{ route('tour.attribute.terms.list', $attribute->id) }}">{{translate('Manage Terms')}}</a>
                                    </div>
                                </td>
                            </tr>
                        @endforeach
    ...
    @include('backend.tours.attributes.modal')
@endsection
