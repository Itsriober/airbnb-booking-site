        ...
        <div class="page-title d-flex justify-content-between align-items-center">
            ...
            <div class="language-changer">
                ...
                @foreach (\App\Models\Language::all() as $key => $language)
                    ...
                    @else
                        <a
                            href="{{ route('tour.attribute.terms.edit', ['attribute_id' => $attribute_id, 'id' => $termSingle->id, 'lang' => $language->code]) }}"><img
                                src="{{ asset('assets/img/flags/' . $language->code . '.png') }}" class="mr-3"
                                height="16"></a>
                    @endif
                @endforeach
            </div>
            <a href="{{ route('tour.attribute.terms.list', $attribute_id) }}" class="eg-btn btn--primary back-btn"> <img
    ...
    <div class="row">
        <div class="col-lg-8 mx-auto">
            <div class="eg-card product-card">
                ...
                <form
                    action="{{ route('tour.attribute.terms.update', ['attribute_id' => $attribute_id, 'id' => $termSingle->id]) }}"
                    ...
                    <div class="form-inner mb-35">
                        ...
                        <input type="text" value="{{ old('name', $termSingle->getTranslation('name', $lang)) }}"
                    ...
                    <div class="form-inner mb-25">
                        ...
                        <input type="text" value="{{ old('icon', $termSingle->icon) }}" name="icon"
                            class="icon icon-picker" placeholder="Ex: bi bi-facebook">
                        @error('icon')
                            <div class="error text-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    @if ($termSingle->image)
                        <img src="{{ asset('uploads/tour/attribute/' . $termSingle->image) }}" alt="{{ $termSingle->name }}"
                            width="50">
                    @endif
