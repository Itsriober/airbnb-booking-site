                        ...
                        @csrf
                                ...
                                <div class="form-inner mb-35 row">
                                        ...
                                        <select class="js-example-basic-single" name="parent_id">
                                            ...
                                            <option value="{{$pcat->id}}" {{ old('parent_id',$categorySingle->parent_id) == $pcat->id ? 'selected' : '' }}>{{$pcat->name}}</option>
                                            @if($pcat->child)
                                            @foreach($pcat->child as $child)
                                            <option value="{{$child->id}}" {{ old('parent_id',$categorySingle->parent_id) == $child->id ? 'selected' : '' }}>--{{$child->name}}</option>
                                            @if($child->child)
                                            @foreach($child->child as $child2)
                                            <option value="{{$child2->id}}" {{ old('parent_id',$categorySingle->parent_id) == $child2->id ? 'selected' : '' }}>----{{$child2->name}}</option>
                                            @endforeach
                                            @endif
                                            @endforeach
                                            @endif
